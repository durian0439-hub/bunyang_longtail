...
class TestNaverBundlePublish(unittest.TestCase):
    def test_prod_cron_runs_tax_and_loan_after_cheongyak_and_auction(self) -> None:
        ...
        loan_pos = script.index("'domain': 'loan'")
        ...
        self.assertLess(tax_pos, loan_pos)
        self.assertLess(loan_pos, loop_pos)
        ...
        self.assertIn("LONGTAIL_LOAN_NAVER_CATEGORY_NO", script)
        self.assertIn("'category_no': os.environ.get('LONGTAIL_LOAN_NAVER_CATEGORY_NO', '19').strip()", script)

    def test_loan_publish_bundle_uses_loan_disclaimer_and_tags(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            result = build_publish_bundle(
                bundle_id=9,
                variant_title="잔금대출 한도 입주 직전, 실수요자가 먼저 볼 기준",
                article_markdown="""# 잔금대출 한도 입주 직전, 실수요자가 먼저 볼 기준

## 상단 요약
입주 직전 잔금대출은 소득, 기존 대출, 주택 수, 실행일을 먼저 확인해야 합니다.

## FAQ
Q. 계산기 한도만 보면 되나요?
A. 실제 한도는 은행 심사로 달라질 수 있습니다.
""",
                output_root=Path(temp_dir),
                image_provider="local",
                domain="loan",
            )
        self.assertIn("부동산대출", result.tags)
        self.assertIn("주택담보대출", result.tags)
        self.assertIn("대출 한도와 금리", result.markdown)
        self.assertNotIn("입주자모집공고와 사업주체", result.markdown)
...
"""
    ...
    def test_build_publish_markdown_inserts_az_hub_as_naver_card_url(self) -> None:
        _, sections = parse_publish_sections(
            SPARSE_CASHFLOW_ARTICLE,
            title_hint="분양 계약금 중도금 잔금, 실제 필요한 현금은 얼마일까?",
        )
        ...
        self.assertIn("전체 목차 보기\n부동산 공부 A-Z 전체 목차\nhttps://blog.naver.com/example/az-hub", markdown)
        self.assertNotIn("[부동산 공부 A-Z 전체 목차](https://blog.naver.com/example/az-hub)", markdown)
